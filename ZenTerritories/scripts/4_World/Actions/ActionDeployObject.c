...
{			 
	...
	protected bool CanIPlaceHere(EntityAI kit, EntityAI item, vector pos, string GUID = "")
	{
		...
		if (!GetZenTerritoriesConfig().CanBuildHere(pos, item.GetType()) || !GetZenTerritoriesConfig().CanBuildHere(pos, kit.GetType()))
		{
			GetZenTerritoriesConfig().SendNotification(GetZenTerritoriesConfig().NoBuildZoneMessage, TerritoryIcons.NoBuildZone);
			m_CanPlaceHere = false;
			return m_CanPlaceHere;
		} else 
		if (GetZenTerritoriesConfig().IsInWhiteList(item.GetType()) || GetZenTerritoriesConfig().IsInWhiteList(kit.GetType()))
		{
			m_CanPlaceHere = true;
			return m_CanPlaceHere;
		} else 
		{
			...
			{
				GetGame().GetObjectsAtPosition(pos, GameConstants.REFRESHER_RADIUS * 1.2, objects, null);
				Object obj;
				...
				for (int x = 0; x < objects.Count(); x++)
				{
					...
					{
						GetZenTerritoriesConfig().SendNotification("Sorry, you can only build one cabin per territory!", TerritoryIcons.NoBuildZone);
						m_CanPlaceHere = false;
						return m_CanPlaceHere;
					}
				}
			...
			if (objects)
			{
				for (int i = 0; i < objects.Count(); i++)
				{
					if (Class.CastTo(theFlag, objects.Get(i)))
					{
						if (kit.IsInherited(TerritoryFlagKit))
						{
							GetZenTerritoriesConfig().SendNotification(GetZenTerritoriesConfig().TerritoryConflictMessage, TerritoryIcons.TerritoryConflict);
						...
						if (theFlag.HasRaisedFlag())
						{
							...
							if (!m_CanPlaceHere)
							{
								GetZenTerritoriesConfig().SendNotification(GetZenTerritoriesConfig().WithinTerritoryWarning, TerritoryIcons.WithinTerritory);
							}
		...
		if (GetZenTerritoriesConfig().RequireTerritory)
		{
			GetZenTerritoriesConfig().SendNotification(GetZenTerritoriesConfig().TerritoryRequiredWarningMessage, TerritoryIcons.TerritoryRequiredWarning);
			m_CanPlaceHere = false;
			
		} else 
		{
			string DeSpawnWarningMessage = GetZenTerritoriesConfig().DeSpawnWarningMessage;
			int LifeTime = 0;
			int itemLifetime = 0;
			ItemBase theItem;
			ItemBase theKit;
			...
			itemLifetime = GetZenTerritoriesConfig().GetKitLifeTime(theItem.GetType());
			...
			LifeTime = GetZenTerritoriesConfig().GetKitLifeTime(theKit.GetType());
			...
			GetZenTerritoriesConfig().GetKitLifeTime(kit.GetType());
			string NiceExpireTime = GetZenTerritoriesConfig().NiceExpireTime(LifeTime);
			if (NiceExpireTime != "")
			{
				DeSpawnWarningMessage.Replace("$LIFETIME$", NiceExpireTime);
				DeSpawnWarningMessage.Replace("$ITEMNAME$", ItemName); 
				GetZenTerritoriesConfig().SendNotification(DeSpawnWarningMessage, TerritoryIcons.DeSpawnWarning);
			}
