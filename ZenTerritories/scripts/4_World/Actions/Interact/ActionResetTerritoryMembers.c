class ActionResetTerritoryMembers extends ActionInteractBase
{
	...
	{	
		...
		if (ident && theFlag)
		{
			...
			if (!theFlag.CanReceiveNewOwner() && GetZenTerritoriesConfig().ServerAdmins.Find(ident.GetId()) != -1)
			{
				CanResetOwner = true;
				return true;
			}
	...
	override void OnExecuteServer( ActionData action_data )
	{
		if (action_data && action_data.m_Target && action_data.m_Player)
		{
			...
			if (theFlag && thePlayer && thePlayer.GetIdentity())
			{
				...
				{
					thePlayer.Zen_SendMessage("You claimed a new territory! Other survivors cannot build near you, and your base will persist longer while the flag is up.");
