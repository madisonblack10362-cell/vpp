		...
		/*if (loggedInsideTerritory)
		{
			...
			for (int i = 0; i < 1000; i++)
			{
				...
				if (!DayZPhysics.RayCastBullet(movePos, to, collisionLayerMask, null, hitObject, hitPosition, hitNormal, hitFraction))
				{
					...
					ZenLogger.Log("Territories", "territory", "Moved player out of territory: " + player.GetIdentity().GetName() + " (" + player.GetIdentity().GetPlainId() + ") - took " + i + " roof checks");
					...
					ZenLogger.Log("Territories", "territory", "Can't move player out of territory - attempts=" + i);
			}
		} */
	}
