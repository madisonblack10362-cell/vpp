class ActionTerritoryJoin extends ActionInteractBase
{
	bool IsMember = false;

	void ActionTerritoryJoin()
	{
	...
	override string GetText()
	{
		if (IsMember)
		{
			return "Покинуть территорию";
		}

		return "Вступить в территорию";
	}
	...
	override void OnExecuteServer(ActionData action_data)
	{
		...
		if (theFlag && thePlayer && thePlayer.GetIdentity())
		{
			if (!theFlag.IsTerritoryMember(thePlayer.GetIdentity().GetId()))
			{
				...
				thePlayer.Zen_SendMessage("Вы приняты в территорию!");
			}
			else
			{
				theFlag.RemoveMember(thePlayer.GetIdentity().GetId());
				thePlayer.Zen_SendMessage("Вы покинули территорию.");
			}
		}
	}
};
